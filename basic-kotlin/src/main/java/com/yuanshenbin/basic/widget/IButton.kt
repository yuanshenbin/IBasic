...
class IButton : AppCompatButton {
    ...
    private var i_text_color = 0 //字体颜色
    ...
    private var i_text_color_unselected = 0 //字体颜色
    private var i_text_style = 0 //字体粗
    ...
    constructor(context: Context?, attrs: AttributeSet?) : super(context, attrs) {
        if (attrs != null) {
            val ta = getContext().obtainStyledAttributes(attrs, R.styleable.IButton)
            i_text_style = ta.getInt(R.styleable.IButton_i_text_style, 0)
            isSelectedState = ta.getBoolean(R.styleable.IButton_i_selected, true)
            i_text_color = ta.getColor(R.styleable.IButton_i_text_color, 0)
            i_normal_color = ta.getColor(R.styleable.IButton_i_normal_color, 0)
            i_enabled_color = ta.getColor(R.styleable.IButton_i_enabled_color, 0)
            i_stroke_color = ta.getColor(R.styleable.IButton_i_stroke_color, 0)
            i_solid_color = ta.getColor(R.styleable.IButton_i_solid_color, 0)
            i_pressed_color = ta.getColor(R.styleable.IButton_i_pressed_color, 0)
            i_radius_size = ta.getDimension(R.styleable.IButton_i_radius_size, 0f).toInt()
            i_stroke_width = ta.getDimension(R.styleable.IButton_i_stroke_width, 0f).toInt()
            i_text_color_unselected = ta.getColor(R.styleable.IButton_i_text_color_unselected, 0)
            i_pressed_color_unselected = ta.getColor(R.styleable.IButton_i_pressed_color_unselected, 0)
            i_normal_color_unselected = ta.getColor(R.styleable.IButton_i_normal_color_unselected, 0)
            i_enabled_color_unselected = ta.getColor(R.styleable.IButton_i_enabled_color_unselected, 0)
            i_stroke_color_unselected = ta.getColor(R.styleable.IButton_i_stroke_color_unselected, 0)
            i_solid_color_unselected = ta.getColor(R.styleable.IButton_i_solid_color_unselected, 0)
            i_stroke_width_unselected = ta.getDimension(R.styleable.IButton_i_stroke_width_unselected, 0f).toInt()
            ta.recycle()
        }
