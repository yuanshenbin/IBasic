...
class ProgressWheelWidget : View {
    ...
    constructor(context: Context, attrs: AttributeSet?) : super(context, attrs) {
        parseAttributes(context.obtainStyledAttributes(attrs,
                R.styleable.ProgressWheelWidget))
        setAnimationEnabled()
    }
    ...
    private fun setAnimationEnabled() {
        ...
        animationValue = if (currentApiVersion >= Build.VERSION_CODES.JELLY_BEAN_MR1) {
            Settings.Global.getFloat(context
                    .contentResolver,
                    Settings.Global.ANIMATOR_DURATION_SCALE, 1f)
        } else {
            Settings.System.getFloat(context
                    .contentResolver,
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        val viewWidth = (circleRadius + this.paddingLeft
                + this.paddingRight)
        val viewHeight = (circleRadius + this.paddingTop
                + this.paddingBottom)
    ...
    private fun setupBounds(layout_width: Int, layout_height: Int) {
        ...
        circleBounds = if (!fillRadius) {
            ...
            val circleDiameter = Math.min(minValue, circleRadius * 2 - barWidth
                    * 2)
            ...
            RectF((xOffset + barWidth).toFloat(), (yOffset + barWidth).toFloat(),
                    (xOffset + circleDiameter - barWidth).toFloat(), (yOffset
                    + circleDiameter - barWidth).toFloat())
        } else {
            RectF((paddingLeft + barWidth).toFloat(), (paddingTop
                    + barWidth).toFloat(), (layout_width - paddingRight - barWidth).toFloat(),
    ...
    private fun parseAttributes(a: TypedArray) {
        ...
        val metrics = context.resources
                .displayMetrics
        barWidth = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                barWidth.toFloat(), metrics).toInt()
        rimWidth = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                rimWidth.toFloat(), metrics).toInt()
        circleRadius = TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, circleRadius.toFloat(), metrics).toInt()
        circleRadius = a.getDimension(
                R.styleable.ProgressWheelWidget_matProg_circleRadius, circleRadius.toFloat()).toInt()
        fillRadius = a.getBoolean(R.styleable.ProgressWheelWidget_matProg_fillRadius,
                false)
        barWidth = a.getDimension(
                R.styleable.ProgressWheelWidget_matProg_barWidth, barWidth.toFloat()).toInt()
        rimWidth = a.getDimension(
                R.styleable.ProgressWheelWidget_matProg_rimWidth, rimWidth.toFloat()).toInt()
        val baseSpinSpeed = a
                .getFloat(R.styleable.ProgressWheelWidget_matProg_spinSpeed,
                        spinSpeed / 360.0f)
        spinSpeed = baseSpinSpeed * 360
        barSpinCycleTime = a.getInt(
                R.styleable.ProgressWheelWidget_matProg_barSpinCycleTime,
                barSpinCycleTime.toInt()).toDouble()
        barColor = a.getColor(R.styleable.ProgressWheelWidget_matProg_barColor,
                barColor)
        rimColor = a.getColor(R.styleable.ProgressWheelWidget_matProg_rimColor,
                rimColor)
        linearProgress = a.getBoolean(
                R.styleable.ProgressWheelWidget_matProg_linearProgress, false)
        if (a.getBoolean(
                        R.styleable.ProgressWheelWidget_matProg_progressIndeterminate, false)) {
            spin()
        }
    ...
    private fun updateBarLength(deltaTimeInMilliSeconds: Long) {
        if (pausedTimeWithoutGrowing >= pauseGrowingTime) {
            ...
            val distance = Math.cos((timeStartGrowing
                    / barSpinCycleTime + 1)
                    * Math.PI).toFloat() / 2 + 0.5f
    ...
    /**
     * Reset the number (in increment mode)
     */
    fun resetCount() {
        mProgress = 0.0f
    ...
    private fun runCallback() {
        if (callback != null) {
            val normalizedProgress = Math
                    .round(mProgress * 100 / 360.0f).toFloat() / 100
    ...
    companion object {
        private val TAG = ProgressWheelWidget::class.java.simpleName
    }
}